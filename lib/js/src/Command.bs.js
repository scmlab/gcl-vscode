...
function load(state) {
  console.log("load!");
  View$GclVscode.activate(state);
  return $$Promise.get(Process$AgdaMode.PathSearch.run("gcl"), (function (param) {
                if (param.tag) {
                  var match = Curry._1(Process$AgdaMode.PathSearch.$$Error.toString, param[0]);
                  var body = match[1];
                  var header = match[0];
                  return Belt_Option.forEach(state.panel, (function (__x) {
                                return View$GclVscode.postMessage(__x, /* Display */Caml_chrome_debugger.simpleVariant("Display", [
                                              header,
                                              body
                                            ]));
                              }));
