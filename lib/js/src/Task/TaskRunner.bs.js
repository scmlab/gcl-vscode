...
function Impl(Editor) {
  ...
  var run = function (tasks, state) {
    var runTask = function (task) {
      if (typeof task === "number") {
        if (task === /* Connect */0) {
          return $$Promise.flatMap(Curry._1(State.connect, state), (function (param) {
                        if (param.tag) {
                          var match = Sig$Guacamole.$$Error.toString(param[0]);
                          return run(/* :: */Caml_chrome_debugger.simpleVariant("::", [
                          ...
                          return run(Curry._1(TaskCommand.dispatch, /* Reload */2), state);
                        }
                      }));
        } else {
        ...
        switch (task.tag | 0) {
          case /* WithState */0 :
              return $$Promise.flatMap(Curry._1(task[0], state), (function (tasks) {
                            return run(tasks, state);
                          }));
          case /* MarkError */1 :
          ...
          case /* DispatchCommand */4 :
              ...
              return run(Curry._1(TaskCommand.dispatch, command), state);
          case /* SendRequest */5 :
              ...
              return $$Promise.flatMap(Curry._2(State.sendRequest, state, task[0]), (function (param) {
                            if (param.tag) {
                              ...
                              return run(Curry._1(TaskResponse.handle, param[0]), state);
                            }
                          }));
          case /* Display */6 :
