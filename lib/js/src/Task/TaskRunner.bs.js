...
var GCLImpl$Guacamole = require("../API/GCLImpl.bs.js");
...
function Impl(Editor) {
  ...
  var GCLImpl = GCLImpl$Guacamole.Impl(Editor);
  var runTask = function (task, state) {
    if (typeof task === "number") {
      ...
      switch (task.tag | 0) {
        ...
        case /* MarkError */6 :
            var range = Curry._3(Response$Guacamole.$$Error.Site.toRange, task[0], state.specifications, GCLImpl.Loc.toRange);
            var decorations = Curry._3(Editor.Decoration.highlightBackground, state.editor, "inputValidation.errorBackground", [range]);
            state.decorations = state.decorations.concat([decorations]);
            return $$Promise.resolved(/* [] */0);
        case /* MarkSpec */7 :
            var spec = task[0];
            var range$1 = Curry._1(GCLImpl.Loc.toRange, spec.loc);
            ...
            var decorations$1 = [
              Curry._4(Editor.Decoration.overlayText, state.editor, "descriptionForeground", isQQ ? "" : preCondText, startRange),
              Curry._4(Editor.Decoration.overlayText, state.editor, "descriptionForeground", postCondText, endRange),
              Curry._3(Editor.Decoration.highlightBackground, state.editor, "editor.wordHighlightStrongBackground", [startRange]),
              Curry._3(Editor.Decoration.highlightBackground, state.editor, "editor.wordHighlightStrongBackground", [endRange])
            ];
            state.decorations = state.decorations.concat(decorations$1);
            return $$Promise.resolved(/* [] */0);
        case /* DigHole */8 :
            var range$2 = Curry._3(Response$Guacamole.$$Error.Site.toRange, task[0], state.specifications, GCLImpl.Loc.toRange);
            var editor = state.editor;
            var indent = " ".repeat(Curry._1(Editor.Point.column, Curry._1(Editor.$$Range.start, range$2)));
            var holeText = "{!\n" + (indent + ("\n" + (indent + "!}")));
            var holeRange = Curry._2(Editor.$$Range.make, Curry._1(Editor.$$Range.start, range$2), Curry._3(Editor.Point.translate, Curry._1(Editor.$$Range.start, range$2), 0, 1));
            return $$Promise.map(Curry._3(Editor.replaceText, Curry._1(Editor.getDocument, editor), holeRange, holeText), (function (param) {
                          var selectionRange = Curry._2(Editor.$$Range.make, Curry._3(Editor.Point.translate, Curry._1(Editor.$$Range.start, range$2), 1, 0), Curry._3(Editor.Point.translate, Curry._1(Editor.$$Range.start, range$2), 1, 0));
                          Curry._2(Editor.selectText, editor, selectionRange);
                          return /* [] */0;
                        }));
