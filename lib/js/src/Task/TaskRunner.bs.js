...
function runTask(task, state) {
  if (typeof task === "number") {
    if (task === /* Connect */0) {
      return $$Promise.resolved(Task__Command$Guacamole.dispatch(/* Reload */2));
    }
    ...
    switch (task.TAG | 0) {
      ...
      case /* SendRequest */2 :
          ...
          return $$Promise.map(State$Guacamole.sendRequest(state, task._0), (function (error) {
                        if (!error.TAG) {
                          return Task__Response$Guacamole.handle(error._0);
                        }
                        var match = Error$Guacamole.toString(error._0);
                        return {
                                hd: {
                                  TAG: 3,
                                  _0: {
      ...
      case /* Display */3 :
          return $$Promise.map(State$Guacamole.display(state, task._0, task._1), (function (param) {
                        return /* [] */0;
                      }));
      case /* ViewRequest */4 :
          return $$Promise.map(State$Guacamole.sendRequestToView(state, task._0), (function (param) {
                        return /* [] */0;
                      }));
      case /* ViewResponse */5 :
          return $$Promise.resolved(Task__View$Guacamole.handle(state.editor, task._0));
      case /* MarkError */6 :
          var range = Curry._3(Response$Guacamole.$$Error.Site.toRange, task._0, state.specifications, GCL$Guacamole.Loc.toRange);
          var decorations = Editor$AgdaModeVscode.Decoration.highlightBackground(state.editor, "inputValidation.errorBackground", [range]);
      ...
      case /* MarkSpec */7 :
          ...
          var decorations$1 = [
            Editor$AgdaModeVscode.Decoration.overlayText(state.editor, "descriptionForeground", isQQ ? "" : preCondText, startRange),
            Editor$AgdaModeVscode.Decoration.overlayText(state.editor, "descriptionForeground", postCondText, endRange),
            Editor$AgdaModeVscode.Decoration.highlightBackground(state.editor, "editor.wordHighlightStrongBackground", [startRange]),
            Editor$AgdaModeVscode.Decoration.highlightBackground(state.editor, "editor.wordHighlightStrongBackground", [endRange])
          ];
          state.decorations = state.decorations.concat(decorations$1);
          return $$Promise.resolved(/* [] */0);
      case /* DigHole */8 :
          var range$2 = Curry._3(Response$Guacamole.$$Error.Site.toRange, task._0, state.specifications, GCL$Guacamole.Loc.toRange);
          var editor = state.editor;
          var indent = " ".repeat(range$2.start.character);
          var holeText = "{!\n" + (indent + ("\n" + (indent + "!}")));
          var holeRange = new Vscode.Range(range$2.start, range$2.start.translate(0, 1));
          return $$Promise.map(Editor$AgdaModeVscode.$$Text.replace(editor.document, holeRange, holeText), (function (param) {
                        var selectionRange = new Vscode.Range(range$2.start.translate(1, 0), range$2.start.translate(1, 0));
                        Editor$AgdaModeVscode.$$Selection.set(editor, selectionRange);
                        return /* [] */0;
                      }));
