...
function handleResponse(response) {
  switch (response.TAG | 0) {
    case /* Res */0 :
        ...
        Belt_Option.mapWithDefault(Registry$Guacamole.get(response._0), $$Promise.resolved(undefined), (function (state) {
    ...
    case /* CannotDecodeResponse */1 :
        $$Promise.map(Vscode.window.showErrorMessage("Client Internal Error\nCannot decode response from the server\n" + response._0 + "\n" + JSON.stringify(response._1)), (function (param) {
                
              }));
        break;
    case /* CannotDecodeRequest */2 :
        $$Promise.map(Vscode.window.showErrorMessage("Server Internal Error\nCannot decode request from the client\n" + response._0), (function (param) {
                ...
                if (typeof response === "number") {
                  switch (response) {
                    case /* ExportProofObligations */0 :
                        $$Promise.flatMap(Curry._1(state.lspSendRequest, /* ExportProofObligations */1), (function (param) {
                  ...
                  if (response.TAG) {
                    ...
                    $$Promise.flatMap(Curry._1(state.lspSendRequest, {
                              TAG: 2,
                              _0: response._0,
                              _1: response._1,
                              _2: response._2,
                  ...
                  switch (loc.TAG | 0) {
                    case /* MouseOver */0 :
                        var loc$1 = loc._0;
                        console.log("viewOnResponse.MouseOver");
...
function make(param) {
  ...
  var clientOptions = LSP$Guacamole.LanguageClientOptions.make(documentSelector, synchronize, LSP$Guacamole.ErrorHandler.makeDefault("Guacamole", 3));
...
function activate(context) {
  ...
  subscribe(onOpenEditor(function (editor) {
            ...
            var sendRequest = function (kind) {
              var source = editor.document.getText(undefined);
              return send({
                          _0: filePath,
                          _1: source,
                          _2: kind,
            ...
            if (state !== undefined) {
              ...
              var state$1 = State$Guacamole.make(editor, View$Guacamole.send, sendRequest);
              Registry$Guacamole.add(filePath, state$1);
            }
            return $$Promise.get(sendRequest(/* Load */0), handleResponse);
          }));
            ...
            if (shouldTrigger) {
              return Belt_Option.forEach(Registry$Guacamole.get(filePath), (function (state) {
                            return Belt_Option.forEach(Belt_Array.get(selections, 0), (function (selection) {
                                          ...
                                          return $$Promise.get(Curry._1(state.lspSendRequest, {
                                                          TAG: 0,
                                                          _0: start,
                                                          _1: end_,
