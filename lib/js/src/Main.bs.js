...
var handle = {
  ...
  lspSubscription: undefined
};

function start(param) {
  if (Belt_Option.isNone(handle.lspSubscription)) {
    handle.lspSubscription = Caml_option.some(handle.client.start());
    return ;
  }
...
function stop(param) {
  var subscription$prime = handle.lspSubscription;
  ...
  return $$Promise.tap(handle.client.stop(), (function (param) {
                handle.lspSubscription = undefined;
...
function unwire(param) {
  return Belt_Option.forEach(handle.stateSubscription, (function (prim) {
                return prim.dispose();
              }));
}
...
function wire(state) {
  unwire(undefined);
  handle.stateSubscription = Caml_option.some(new Vscode.Disposable(Req$Guacamole.handle(state.connection, sendRequest)));
...
function onOpenEditor(context, editor) {
  ...
  return onActivateExtension(function (param) {
              ...
              Belt_Option.forEach(Registry$Guacamole.get(filePath), (function (state) {
...
function onNotification$1(response) {
...
var Handler = {
  ...
  onNotification: onNotification$1
};
...
function activate(context) {
  ...
  return onNotification(onNotification$1);
}
