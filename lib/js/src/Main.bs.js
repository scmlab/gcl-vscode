...
function handleViewResponse(devMode, response) {
  return Belt_Option.forEach(previouslyActivatedState.contents, (function (state) {
                if (typeof response === "number") {
                  switch (response) {
                    case /* Connect */0 :
                        LSP$Guacamole.start(devMode);
                        return ;
                    case /* Disconnect */1 :
                        LSP$Guacamole.stop(undefined);
                        return ;
                    case /* ExportProofObligations */2 :
                  ...
                  switch (response.TAG | 0) {
                    case /* ChangeConnectionMethod */0 :
                        LSP$Guacamole.changeMethod(response._0);
                        return ;
                    case /* Link */1 :
                        var loc = response._0;
                        switch (loc.TAG | 0) {
...
function activate(context) {
  ...
  subscribe(LSP$Guacamole.onResponse(function (response) {
            if (response.TAG === /* Res */0) {
              console.log(Belt_Array.map(response._1, (function (kind) {
                            switch (kind.TAG | 0) {
                              case /* Error */0 :
                                  return "[Error]";
                              case /* OK */1 :
  ...
  subscribe(LSP$Guacamole.onChangeStatus(function (status) {
            State$Guacamole.updateConnectionStatus(status);
            
          }));
  subscribe(LSP$Guacamole.onChangeMethod(function (method) {
            State$Guacamole.updateConnectionMethod(method);
  ...
  subscribe(View$Guacamole.on(function (param) {
            return handleViewResponse(devMode, param);
          }));
