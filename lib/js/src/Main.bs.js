...
function handleResponse(response) {
  if (typeof response === "number") {
    return State$Guacamole.displayBlocks([Element$Guacamole.Block.block("Internal Error", undefined, Element$Guacamole.Inlines.string("Program source not loaded yet"))]);
  }
  switch (response.TAG | 0) {
    ...
    case /* CannotDecodeResponse */1 :
        return State$Guacamole.displayBlocks([Element$Guacamole.Block.block("Client Internal Error", undefined, Element$Guacamole.Inlines.string("Cannot decode response from the server\n" + response._0 + "\n" + JSON.stringify(response._1)))]);
    case /* CannotDecodeRequest */2 :
        return State$Guacamole.displayBlocks([Element$Guacamole.Block.block("Server Internal Error", undefined, Element$Guacamole.Inlines.string("Cannot decode request from the client\n" + response._0))]);
    case /* CannotSendRequest */3 :
        return State$Guacamole.displayBlocks([Element$Guacamole.Block.block("Client Internal Error", undefined, Element$Guacamole.Inlines.string("Cannot send request to the server\n" + response._0))]);
...
function sendLSPRequest(state, kind) {
  return $$Promise.flatMap(State$Guacamole.sendLSPRequest(state, kind), (function (result) {
                ...
                return State$Guacamole.displayBlocks([Element$Guacamole.Block.block(match[0], undefined, Element$Guacamole.Inlines.string(match[1]))]);
              }));
}
  ...
  return Belt_Option.forEach(previouslyActivatedState.contents, (function (state) {
                if (typeof response === "number") {
                  ...
                  switch (loc.TAG | 0) {
                    ...
                    case /* MouseClick */2 :
                        var loc$2 = loc._0;
                        var key$1 = GCL$Guacamole.Loc.toString(loc$2);
                        State$Guacamole.Decoration.remove(key$1);
                        State$Guacamole.focus(state);
                        var range$1 = GCL$Guacamole.Loc.toRange(loc$2);
                        var selection = new Vscode.Selection(range$1.start, range$1.end);
...
function activate(context) {
  ...
  subscribe(Connection$Guacamole.onResponse(function (result) {
            ...
            var match = Connection__Error$Guacamole.toString(result._0);
            State$Guacamole.displayBlocks([Element$Guacamole.Block.block(match[0], undefined, Element$Guacamole.Inlines.string(match[1]))]);
