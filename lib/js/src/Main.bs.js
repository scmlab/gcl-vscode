                ...
                if (typeof response === "number") {
                  ...
                  switch (response.TAG | 0) {
                    case /* Connect */0 :
                        ...
                        if (LSP$Guacamole.isConnected(undefined)) {
                          $$Promise.flatMap(LSP$Guacamole.stop(undefined), (function (param) {
                                  return LSP$Guacamole.start(devMode, viaTCP);
                                }));
                        } else {
                          LSP$Guacamole.start(devMode, viaTCP);
                        }
                        return ;
                    case /* Link */1 :
                        var loc = response._0;
...
function activate(context) {
  ...
  subscribe(LSP$Guacamole.onResponse(function (response) {
            handleResponse(response);
            
          }));
  subscribe(LSP$Guacamole.onChangeConnectionStatus(function (status) {
            switch (status) {
              case /* Disconnected */0 :
  ...
  subscribe(LSP$Guacamole.onError(function (exn) {
            ...
            if (shouldSwitchToSTDIO) {
              LSP$Guacamole.start(false, false);
            } else {
