...
function handleViewResponse(devMode, response) {
  return Belt_Option.forEach(previouslyActivatedState.contents, (function (state) {
                if (typeof response === "number") {
                  ...
                  switch (response.TAG | 0) {
                    case /* Connect */0 :
                        var viaTCP = response._0;
                        if (LSP$Guacamole.Client.isConnected(undefined)) {
                          $$Promise.flatMap(LSP$Guacamole.Client.stop(undefined), (function (param) {
                                  return LSP$Guacamole.Client.start(devMode, viaTCP);
                                }));
                        } else {
                          LSP$Guacamole.Client.start(devMode, viaTCP);
                        }
                        return ;
                    case /* Link */1 :
                        var loc = response._0;
...
function activate(context) {
  ...
  subscribe(LSP$Guacamole.Client.on(function (response) {
            handleResponse(response);
            
          }));
  subscribe(LSP$Guacamole.Client.onChangeConnectionStatus(function (status) {
            switch (status) {
              case /* Disconnected */0 :
                  State$Guacamole.updateConnectionStatus(/* Disconnected */0);
                  break;
              case /* Connecting */1 :
                  State$Guacamole.updateConnectionStatus(/* Connecting */1);
                  break;
              case /* Connected */2 :
                  State$Guacamole.updateConnectionStatus(/* Connected */2);
                  break;
              
            }
            
          }));
  subscribe(LSP$Guacamole.Client.onError(function (exn) {
            ...
            var shouldSwitchToSTDIO = false && isECONNREFUSED;
            if (shouldSwitchToSTDIO) {
              LSP$Guacamole.Client.start(false, false);
            } else {
              State$Guacamole.displayErrorMessages(messages);
            }
  ...
  subscribe(onActivateExtension(function (param) {
            ...
            return $$Promise.get(View$Guacamole.activate(extensionPath, false), (function (_viewActivationResult) {
                          LSP$Guacamole.Client.start(false, false);
  ...
  subscribe(View$Guacamole.on(function (param) {
            return handleViewResponse(false, param);
          }));
