// Generated by ReScript, PLEASE EDIT WITH CARE
'use strict';

var Curry = require("rescript/lib/js/curry.js");
var Caml_option = require("rescript/lib/js/caml_option.js");
var Json_decode = require("@glennsl/bs-json/lib/js/src/Json_decode.bs.js");
var Json_encode = require("@glennsl/bs-json/lib/js/src/Json_encode.bs.js");
var Link$Guabao = require("./Link.bs.js");
var Util$Guabao = require("../Util/Util.bs.js");
var Element__Inlines$Guabao = require("./Element/Element__Inlines.bs.js");
var Element__Section$Guabao = require("./Element/Element__Section.bs.js");

var decode = Util$Guabao.Decode.sum(function (x) {
      switch (x) {
        case "Display" :
            return {
                    TAG: /* Contents */0,
                    _0: (function (param) {
                        return Json_decode.map((function (param) {
                                      return {
                                              TAG: /* Display */2,
                                              _0: param[0],
                                              _1: param[1]
                                            };
                                    }), (function (param) {
                                      return Json_decode.tuple2(Json_decode.$$int, (function (param) {
                                                    return Json_decode.array(Element__Section$Guabao.decode, param);
                                                  }), param);
                                    }), param);
                      })
                  };
        case "Substitute" :
            return {
                    TAG: /* Contents */0,
                    _0: (function (param) {
                        return Json_decode.map((function (param) {
                                      return {
                                              TAG: /* Substitute */1,
                                              _0: param[0],
                                              _1: param[1]
                                            };
                                    }), (function (param) {
                                      return Json_decode.tuple2(Json_decode.$$int, Element__Inlines$Guabao.decode, param);
                                    }), param);
                      })
                  };
        case "UpdateConnectionStatus" :
            return {
                    TAG: /* Contents */0,
                    _0: (function (param) {
                        return Json_decode.map((function (method) {
                                      return {
                                              TAG: /* UpdateConnectionStatus */0,
                                              _0: method
                                            };
                                    }), Json_decode.string, param);
                      })
                  };
        default:
          throw {
                RE_EXN_ID: Json_decode.DecodeError,
                _1: "[Request] Unknown constructor: " + x,
                Error: new Error()
              };
      }
    });

function encode(x) {
  switch (x.TAG | 0) {
    case /* UpdateConnectionStatus */0 :
        return Json_encode.object_({
                    hd: [
                      "tag",
                      "UpdateConnectionStatus"
                    ],
                    tl: {
                      hd: [
                        "contents",
                        x._0
                      ],
                      tl: /* [] */0
                    }
                  });
    case /* Substitute */1 :
        return Json_encode.object_({
                    hd: [
                      "tag",
                      "Substitute"
                    ],
                    tl: {
                      hd: [
                        "contents",
                        Json_encode.tuple2((function (prim) {
                                return prim;
                              }), Element__Inlines$Guabao.encode, [
                              x._0,
                              x._1
                            ])
                      ],
                      tl: /* [] */0
                    }
                  });
    case /* Display */2 :
        return Json_encode.object_({
                    hd: [
                      "tag",
                      "Display"
                    ],
                    tl: {
                      hd: [
                        "contents",
                        Json_encode.tuple2((function (prim) {
                                return prim;
                              }), (function (param) {
                                return Json_encode.array(Element__Section$Guabao.encode, param);
                              }), [
                              x._0,
                              x._1
                            ])
                      ],
                      tl: /* [] */0
                    }
                  });
    
  }
}

var $$Request = {
  decode: decode,
  encode: encode
};

var decode$1 = Util$Guabao.Decode.sum(function (x) {
      switch (x) {
        case "Destroyed" :
            return {
                    TAG: /* TagOnly */1,
                    _0: (function (param) {
                        return /* Destroyed */1;
                      })
                  };
        case "Initialized" :
            return {
                    TAG: /* TagOnly */1,
                    _0: (function (param) {
                        return /* Initialized */0;
                      })
                  };
        case "InsertAnchor" :
            return {
                    TAG: /* Contents */0,
                    _0: (function (json) {
                        return {
                                TAG: /* InsertAnchor */1,
                                _0: Json_decode.string(json)
                              };
                      })
                  };
        case "Link" :
            return {
                    TAG: /* Contents */0,
                    _0: (function (json) {
                        return {
                                TAG: /* Link */0,
                                _0: Curry._1(Link$Guabao.$$Event.decode, json)
                              };
                      })
                  };
        case "Solve" :
            return {
                    TAG: /* Contents */0,
                    _0: (function (json) {
                        return {
                                TAG: /* Solve */2,
                                _0: Caml_option.null_to_opt(Json_decode.nullable(Json_decode.string, json))
                              };
                      })
                  };
        case "Substitute" :
            return {
                    TAG: /* Contents */0,
                    _0: (function (json) {
                        return {
                                TAG: /* Substitute */3,
                                _0: Json_decode.$$int(json)
                              };
                      })
                  };
        default:
          throw {
                RE_EXN_ID: Json_decode.DecodeError,
                _1: "[Response.t] Unknown constructor: " + x,
                Error: new Error()
              };
      }
    });

function encode$1(x) {
  if (typeof x === "number") {
    if (x === /* Initialized */0) {
      return Json_encode.object_({
                  hd: [
                    "tag",
                    "Initialized"
                  ],
                  tl: /* [] */0
                });
    } else {
      return Json_encode.object_({
                  hd: [
                    "tag",
                    "Destroyed"
                  ],
                  tl: /* [] */0
                });
    }
  }
  switch (x.TAG | 0) {
    case /* Link */0 :
        return Json_encode.object_({
                    hd: [
                      "tag",
                      "Link"
                    ],
                    tl: {
                      hd: [
                        "contents",
                        Link$Guabao.$$Event.encode(x._0)
                      ],
                      tl: /* [] */0
                    }
                  });
    case /* InsertAnchor */1 :
        return Json_encode.object_({
                    hd: [
                      "tag",
                      "InsertAnchor"
                    ],
                    tl: {
                      hd: [
                        "contents",
                        x._0
                      ],
                      tl: /* [] */0
                    }
                  });
    case /* Solve */2 :
        return Json_encode.object_({
                    hd: [
                      "tag",
                      "Solve"
                    ],
                    tl: {
                      hd: [
                        "contents",
                        Json_encode.nullable((function (prim) {
                                return prim;
                              }), x._0)
                      ],
                      tl: /* [] */0
                    }
                  });
    case /* Substitute */3 :
        return Json_encode.object_({
                    hd: [
                      "tag",
                      "Substitute"
                    ],
                    tl: {
                      hd: [
                        "contents",
                        x._0
                      ],
                      tl: /* [] */0
                    }
                  });
    
  }
}

var $$Response = {
  decode: decode$1,
  encode: encode$1
};

exports.$$Request = $$Request;
exports.$$Response = $$Response;
/* decode Not a pure module */
