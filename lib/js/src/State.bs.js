...
function fromCursorPosition(state) {
  ...
  Belt_Array.forEach(Belt_Array.keep(state.specifications, (function (spec) {
              var range = SrcLoc$Guacamole.Loc.toVSCodeRange(spec.loc);
              return range.contains(cursor);
            })), (function (spec) {
          var spec$prime = smallestHole.contents;
          if (spec$prime !== undefined && !SrcLoc$Guacamole.Loc.toVSCodeRange(spec.loc).contains(SrcLoc$Guacamole.Loc.toVSCodeRange(spec$prime.loc))) {
            return ;
          } else {
