...
function onDownload($$event) {
  ...
  if (typeof $$event === "number") {
    message = $$event !== 0 ? "Downloaded" : "Downloading ...";
  } else {
    var total = $$event._1;
    var accum = $$event._0;
    message = total > 10485760 ? "Downloading ( " + String(accum / 1048576 | 0) + " MB / " + String(total / 1048576 | 0) + " MB )" : "Downloading ( " + String(accum / 1024 | 0) + " KB / " + String(total / 1024 | 0) + " MB )";
  }
  updateConnectionStatus(message);
  
}

function substitute(id, expr) {
  return $$Promise.map(View$Guabao.send({
                  TAG: /* Substitute */1,
                  _0: id,
                  _1: expr
...
function sendLSPRequest(state, kind) {
  return Connection$Guabao.sendRequest(state.globalStoragePath, onDownload, /* Req */{
              _0: state.filePath,
              _1: kind
            });
}

function decorateAndUpdateSpecs(state, specs) {
...
function decorateAndUpdatePOs(state, pos) {
  Belt_Array.forEach(state.proofObligations, (function (po) {
          return Belt_Array.forEach(po.decorations, (function (prim) {
                        prim.dispose();
                        
                      }));
        }));
  state.proofObligations = pos;
  return Belt_Array.forEach(state.proofObligations, (function (po) {
                var range = SrcLoc$Guabao.$$Range.toVSCodeRange(po.range);
                var highlightBackground = function (ranges) {
                  var backgroundColor = VSCode.StringOr.others(new Vscode.ThemeColor("editor.wordHighlightStrongBackground"));
                  var rangeBehavior = VSCode.DecorationRangeBehavior.toEnum(/* ClosedClosed */1);
                  var options = {
                ...
                var decorations = [highlightBackground([range])];
...
function handleResponseKind(state, kind) {
  switch (kind.TAG | 0) {
    ...
    case /* UpdateSpecs */1 :
        decorateAndUpdateSpecs(state, kind._0);
        return $$Promise.resolved(undefined);
    case /* MarkPOs */2 :
        decorateAndUpdatePOs(state, kind._0);
        return $$Promise.resolved(undefined);
    case /* Substitute */3 :
        return substitute(kind._0, kind._1);
    case /* ConsoleLog */4 :
        console.log(kind._0);
        return $$Promise.resolved(undefined);
...
function destroy(state) {
  ...
  Belt_Array.forEach(state.proofObligations, Response$Guabao.ProofObligation.destroy);
...
exports.decorateAndUpdateSpecs = decorateAndUpdateSpecs;
exports.decorateAndUpdatePOs = decorateAndUpdatePOs;
