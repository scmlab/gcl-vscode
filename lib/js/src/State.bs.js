...
var Editor$Guabao = require("./Util/Editor.bs.js");
...
function sendLSPRequest(state, kind) {
  return Connection$Guabao.sendRequest(state.globalStoragePath, {
              _0: state.filePath,
              _1: kind,
              [Symbol.for("name")]: "Req"
            });
...
function fromCursorPosition(state) {
  ...
  Belt_Array.forEach(Belt_Array.keep(state.specifications, (function (spec) {
          ...
          if (spec$prime !== undefined && !SrcLoc$Guabao.$$Range.toVSCodeRange(spec.range).contains(SrcLoc$Guabao.$$Range.toVSCodeRange(spec$prime.range))) {
            return ;
          } else {
...
function resolve(state, i) {
  ...
  return $$Promise.map($$Promise.flatMap(Editor$Guabao.$$Text.$$delete(state.document, range), (function (result) {
                    if (result) {
                      return Editor$Guabao.$$Text.insert(state.document, start, indentedPayload);
                    } else {
...
function updateLocations(state, locations) {
  Belt_Array.forEachWithIndex(state.specifications, (function (index, spec) {
          return Belt_Option.forEach(Belt_Array.get(locations, index), (function (range) {
                        spec.range = range;
