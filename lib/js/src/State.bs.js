...
function fromCursorPosition(state) {
  ...
  Belt_Array.forEach(Belt_Array.keep(state.specifications, (function (spec) {
              var range = SrcLoc$Guacamole.$$Range.toVSCodeRange(spec.range);
              return range.contains(cursor);
            })), (function (spec) {
          var spec$prime = smallestHole.contents;
          if (spec$prime !== undefined && !SrcLoc$Guacamole.$$Range.toVSCodeRange(spec.range).contains(SrcLoc$Guacamole.$$Range.toVSCodeRange(spec$prime.range))) {
            return ;
          } else {
