...
function display(state, header, body) {
  return View$Guacamole.send(state.view, {
              TAG: 1,
              _0: header,
              _1: body,
...
function resolve(state, i) {
  ...
  Belt_Option.forEach(Belt_Array.get(specs, 0), (function (spec) {
          ...
          return $$Promise.get($$Promise.flatMap(Editor$Guacamole.$$Text.$$delete(doc, range), (function (param) {
                            if (param) {
                              return Editor$Guacamole.$$Text.insert(doc, start, payload.trim());
                            } else {
...
function insert(state, lineNo, expr) {
  ...
  var doc = state.editor.document;
  return Editor$Guacamole.$$Text.insert(doc, point, assertion);
}
