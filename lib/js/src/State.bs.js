...
var ViewController$Guacamole = require("./ViewController.bs.js");
...
function destroy(state) {
  ViewController$Guacamole.destroy(state.view);
...
function make(extentionPath, editor) {
  var view = ViewController$Guacamole.make(extentionPath, editor);
  return {
          editor: editor,
          view: view,
          mode: /* WP1 */0,
...
function show(state) {
  return ViewController$Guacamole.show(state.view);
}

function hide(state) {
  return ViewController$Guacamole.hide(state.view);
}

function sendRequestToView(state, request) {
  return ViewController$Guacamole.send(state.view, request);
}

function display(state, header, body) {
  return ViewController$Guacamole.send(state.view, /* Display */Caml_chrome_debugger.variant("Display", 1, [
                header,
                body
              ]));
}
...
function getPayloadRange(doc, spec) {
  ...
  var end_ = doc.lineAt(startingLine).range.end;
...
function resolve(state, i) {
  ...
  Belt_Option.forEach(Belt_Array.get(specs, 0), (function (spec) {
          ...
          workspaceEdit.delete(doc.uri, range, undefined);
          return $$Promise.get($$Promise.flatMap(Vscode.workspace.applyEdit(workspaceEdit), (function (param) {
                            if (!param) {
                              return $$Promise.resolved(false);
                            }
                            var workspaceEdit = new Vscode.WorkspaceEdit();
                            workspaceEdit.insert(doc.uri, start, payload.trim(), undefined);
...
function insert(state, lineNo, expr) {
  ...
  var doc = state.editor.document;
  var workspaceEdit = new Vscode.WorkspaceEdit();
  workspaceEdit.insert(doc.uri, point, assertion, undefined);
  return Vscode.workspace.applyEdit(workspaceEdit);
}
