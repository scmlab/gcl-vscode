...
var Download$Guacamole = require("./Connection/Download.bs.js");
...
function toString(error) {
  switch (error.TAG | 0) {
    ...
    case /* CannotDownloadPrebuilt */2 :
        return [
                "Cannot download prebuilt language server",
                Download$Guacamole.$$Error.toString(error._0)
              ];
...
function make(method) {
  ...
  switch (method.TAG | 0) {
    case /* ViaStdIO */0 :
        serverOptions = LSP$Guacamole.ServerOptions.makeWithCommand(method._0);
        break;
    case /* ViaTCP */1 :
        serverOptions = LSP$Guacamole.ServerOptions.makeWithStreamInfo(method._0);
        break;
    case /* ViaPrebuilt */2 :
        serverOptions = LSP$Guacamole.ServerOptions.makeWithCommand(method._1);
        break;
...
function probe$2(context) {
  return $$Promise.mapError($$Promise.mapOk(Download$Guacamole.get(context), (function (path) {
                    return {
                            TAG: 2,
                ...
                return {
                        TAG: 2,
                        _0: e,
                        [Symbol.for("name")]: "CannotDownloadPrebuilt"
                      };
              }));
}

function probe$3(tryTCP, globalStoragePath) {
  var name = "gcl";
  if (tryTCP) {
    return $$Promise.flatMapError($$Promise.flatMapError(probe$1(3000), (function (param) {
                      return probe$2(globalStoragePath);
                    })), (function (param) {
                  return probe(name);
                }));
  } else {
    return $$Promise.flatMapError(probe$2(globalStoragePath), (function (param) {
                  return probe(name);
                }));
  }
}
...
function make$1(tryTCP, globalStoragePath) {
  ...
  return $$Promise.map($$Promise.flatMapOk(probe$3(tryTCP, globalStoragePath), make), (function (result) {
                if (result.TAG === /* Ok */0) {
...
function sendRequest$1(tryTCP, globalStoragePath, request) {
  var client = singleton.contents;
  if (typeof client === "number") {
    return $$Promise.flatMapOk(make$1(tryTCP, globalStoragePath), (function (param) {
                  return sendRequest$1(tryTCP, globalStoragePath, request);
                }));
  }
  if (client.TAG !== /* Connecting */0) {
