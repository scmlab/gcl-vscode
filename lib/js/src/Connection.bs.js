...
function start(globalStoragePath, onDownload) {
  ...
  return $$Promise.map($$Promise.flatMapOk($$Promise.flatMap(Connection__Probe$Guabao.probe(globalStoragePath, onDownload), (function (param) {
                        var errors = param[1];
                        var result = param[0];
                        if (result !== undefined) {
                          console.log(Belt_Array.map(errors, Source$LanguageServerMule.$$Error.toString));
                          return $$Promise.resolved({
                                      TAG: /* Ok */0,
                                      _0: result
                                    });
                    ...
                    return $$Promise.mapError(Client__LSP$LanguageServerMule.make("guabao", "Guabao Language Server", method), (function (e) {
                                  return {
