...
function start(globalStoragePath, onDownload) {
  ...
  var match$2 = Connection__Probe$Guabao.probe(globalStoragePath, onDownload);
  var result = $$Promise.map($$Promise.flatMapOk($$Promise.mapError(match$2[0], (function (error) {
                  return {
...
function sendRequest(globalStoragePath, onDownload, request) {
  var match = singleton.contents;
  if (typeof match === "number") {
    var match$1 = start(globalStoragePath, onDownload);
    return $$Promise.flatMapOk(match$1[0], (function (param) {
  ...
  var match$2 = $$Promise.pending(undefined);
  match._0.push([
        request,
        match$2[1]
      ]);
