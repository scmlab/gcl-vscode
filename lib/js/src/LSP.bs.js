...
function makeAndStart(devMode, viaTCP) {
  var emittedError = {
    contents: false
  };
...
function start(devMode, viaTCP) {
  ...
  var state = makeAndStart(devMode, viaTCP);
  singleton.contents = {
    TAG: 0,
    _0: state,
  ...
  return $$Promise.map($$Promise.Js.toResult(state.client.onReady()), (function (result) {
                if (result.TAG !== /* Ok */0) {
                  return ;
                }
                Chan$Guacamole.emit(statusChan, /* Connected */2);
                state.client.onNotification("guacamole", (function (json) {
                        return Chan$Guacamole.emit(dataChan, decodeResponse(json));
                      }));
                singleton.contents = {
                  TAG: 1,
                  _0: state,
...
function on(handler) {
  return new Vscode.Disposable(Chan$Guacamole.on(dataChan, handler));
}
