...
function toOutputPath(context, param) {
...
function downloadLanguageServer(context, param) {
  var asset = param[1];
  var outputPath = toOutputPath(context, [
        param[0],
        asset
      ]);
  ...
  return $$Promise.flatMapOk($$Promise.flatMapOk(getWithRedirects(httpOptions), (function (res) {
                    var match = $$Promise.pending(undefined);
                    var resolve = match[1];
                ...
                return run(outputPath + ".zip", outputPath);
              }));
}
