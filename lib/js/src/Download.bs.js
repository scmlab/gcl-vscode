...
function run(src, dest) {
  console.log("[ download ][ unzip ] " + src + " => " + dest);
  var match = $$Promise.pending(undefined);
  var resolve = match[1];
...
function getCurrentReleaseAndAsset(param) {
  ...
  return $$Promise.flatMapOk($$Promise.flatMapOk(getReleasesFromGitHub(undefined), getMatchingRelease), getMatchingAsset);
}

function toDestPath(context, param) {
  var globalStoragePath = context.globalStoragePath;
  var osName = param[1].name.slice(4, -4);
  return Path.join(globalStoragePath, "gcl-" + param[0].tagName + "-" + osName);
}
...
function downloadLanguageServer(context, param) {
  var asset = param[1];
  var destPath = toDestPath(context, [
        param[0],
        asset
      ]);
  return $$Promise.flatMap($$Promise.flatMapOk($$Promise.flatMapOk($$Promise.flatMapOk(download(asset.url, destPath + ".zip.download"), (function (param) {
                            return $$Promise.mapError(rename(destPath + ".zip.download", destPath + ".zip"), (function (e) {
                                          return {
                                                  TAG: 6,
                                                  _0: e,
                        ...
                        return run(destPath + ".zip", destPath);
                      })), (function (param) {
                    return $$Promise.mapError(unlink(destPath + ".zip"), (function (e) {
                                  return {
                                          TAG: 5,
                                          _0: e,
...
function checkExistingDownload(context) {
  var globalStoragePath = context.globalStoragePath;
  ...
  switch (others) {
    case "darwin" :
        getExpectedFileName = {
          TAG: 0,
          _0: "gcl-" + Constant$Guacamole.version + "-macos",
          [Symbol.for("name")]: "Ok"
        };
        break;
    case "linux" :
        getExpectedFileName = {
          TAG: 0,
          _0: "gcl-" + Constant$Guacamole.version + "-ubuntu",
          [Symbol.for("name")]: "Ok"
        };
        break;
    case "win32" :
        getExpectedFileName = {
          TAG: 0,
          _0: "gcl-" + Constant$Guacamole.version + "-windows",
          [Symbol.for("name")]: "Ok"
        };
        break;
    default:
...
var State = {
  checkExistingDownload: checkExistingDownload
};
...
function get(context) {
  ...
  if (match !== undefined) {
    if (match.TAG === /* Downloaded */0) {
      console.log("[ download ] Got existing download");
      return $$Promise.resolved({
                  TAG: 0,
                  _0: match._0,
                  [Symbol.for("name")]: "Ok"
    ...
    console.log("[ download ] Already downloading");
    return match._0;
  }
  var error = checkExistingDownload(context);
  if (error.TAG !== /* Ok */0) {
    return $$Promise.resolved({
                TAG: 1,
                _0: error._0,
  ...
  var path = error._0;
  if (path !== undefined) {
    console.log("[ download ][ init ] Got existing download");
    state.contents = {
      TAG: 0,
      _0: path,
  ...
  console.log("[ download ][ init ] No existing download, fetch a new one");
  var match$1 = $$Promise.pending(undefined);
  state.contents = {
    TAG: 1,
    _0: match$1[0],
    [Symbol.for("name")]: "Downloading"
  };
  return $$Promise.tap($$Promise.flatMapOk(getCurrentReleaseAndAsset(undefined), (function (param) {
                    return downloadLanguageServer(context, param);
                  })), match$1[1]);
}
...
exports.getCurrentReleaseAndAsset = getCurrentReleaseAndAsset;
exports.toDestPath = toDestPath;
